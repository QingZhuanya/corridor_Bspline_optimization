...
class FemPosDeviationSqpOsqpInterface {
 public:
  // typedef std::shared_ptr<FemPosDeviationSqpOsqpInterface> Ptr;
 ...
 private:
  ...
  void SetPrimalWarmStart(const std::vector<std::pair<double, double>>& points,
                          std::vector<c_float>* primal_warm_start); //加载原始点和slack参数，并计算两点之间平均距离
 ...
 private:
  ...
  int num_of_points_ = 0; // n
  int num_of_pos_variables_ = 0;// 2n
  int num_of_slack_variables_ = 0; //n - 2
  int num_of_variables_ = 0; //2n + n -2
  int num_of_variable_constraints_ = 0; //2n + n -2
  int num_of_curvature_constraints_ = 0; //n-2
  int num_of_constraints_ = 0; // 2n + 2n - 4
