...
namespace opt_planner
{
                                             ...
                                             Eigen::MatrixXd &ctrl_pts)
  {
    ...
    // Axy.block(0, 0, 1, 3) =  inirow.transpose();
    for (int i = 0; i < K; ++i)
      Axy.block(i, i, 1, 3) = (1 / 6.0) * prow.transpose();
    // Axy.block(K-1, K-1, 1, 3) =  endrow.transpose();
    ...
    //   Axy.block(i, i, 1, 3) = (1 / 6.0) * prow.transpose();
    // // Axy.block(K-1, K-1, 1, 3) =  endrow.transpose();
    // Axy.block(0, 0, 1, 3) = (1 / 2.0 ) * inirow.transpose();
    // Axy.block(1, 1, 1, 3) = (1 / 2.0 ) * secrow.transpose();
    // Axy.block(K - 2, K - 2 , 1, 3) = (1 / 2.0) * lsecrow.transpose();
    // Axy.block(K - 1, K - 1, 1, 3) = (1 / 2.0) * endrow.transpose();

    // write b
    Eigen::VectorXd bx(K + 4), by(K + 4), bz(K + 4);
    // Eigen::VectorXd bx(K), by(K), bz(K + 4);
    for (int i = 0; i < K; ++i)
    {
      bx(i) = point_set[i](0);
      by(i) = point_set[i](1);
    ...
    // ctrl_pts.row(2) = Eigen::MatrixXd::Zero(1, K + 2);
