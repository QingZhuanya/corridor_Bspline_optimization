...
//   }
  ...
  void BsplineOptimizer::setBsplineCorridor(const VecCube& corridor) {
    ...
    {   
        ...
        {   
            for(int j = 0; j < order_; j ++ ) // 每个waypoint对应的所有控制点
            {   
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    // double step_size = hybrid_astar_->getMapResolution() / ((init_points.col(0) - init_points.rightCols(1)).norm() / (init_points.cols() - 1)) / 2;
    // int in_id, out_id;
    ...
    //   {
    //     occ = hybrid_astar_->getOccupancy(a * init_points.col(i - 1) + (1 - a) * init_points.col(i));
    //     // cout << setprecision(5);
...
//   }
  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
      ...
      /* ---------- optimize ---------- */
      t1 = ros::Time::now();
      int result = lbfgs::lbfgs_optimize(variable_num_, q, &final_cost, BsplineOptimizer::costFunctionRebound, NULL, BsplineOptimizer::earlyExit, this, &lbfgs_params, gradient_norm, iteration_count);
      t2 = ros::Time::now();
      ...
      // calcMeanDistance();
      //update mu
      // for(int i = 0 ; i < this->cps_.size - 4; i++){
      //   //控制点应该要写成路径点的形式
      //   // cout<<"1*********"<<endl;
      //   Eigen::Vector2d temp_a = 1/6 * (cps_.points.col(i) + 2 * cps_.points.col(i+1) - 6 * cps_.points.col(i+2) + 2 * cps_.points.col(i+3) + cps_.points.col(i+4));
      //   // cout<<"2*********"<<endl;
      //   double curvature_max_bound = mu_(i,0) + phr_rho_*(temp_a.dot(temp_a)-curvature_constraint_sqr);
      //   // cout<<"3*********"<<endl;
      //   mu_(i,0) = max(curvature_max_bound, 0.0);
      //   // cout<<"4*********"<<endl;
      // }
      // for(int i = 0 ; i < this->cps_.size - 2; i++){
      //   //控制点应该要写成路径点的形式
      //   // cout<<"1*********"<<endl;
      //   Eigen::Vector2d temp_a = (cps_.points.col(i) - 2 * cps_.points.col(i+1) + cps_.points.col(i+2) );
      //   // cout<<"2*********"<<endl;
      //   double curvature_max_bound = mu_(i,0) + phr_rho_*(temp_a.dot(temp_a)-curvature_constraint_sqr);
      //   // cout<<"3*********"<<endl;
      //   mu_(i,0) = max(curvature_max_bound, 0.0);
      ...
      //   double curvature_Max = abs(max((temp_a.dot(temp_a)-curvature_constraint_sqr),-mu_(i,0)/phr_rho_));
      //   // cout<<"**********curvature_Max*************   " << curvature_Max<<endl;
      //   if(curvature_Max > kkt_1){
      //     kkt_1 = curvature_Max;
      //   }
      // }
      // cout<<"*****kkt_1: *****  " << kkt_1 << endl;
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        // double tm, tmp;
        // // cout << "bspline_interval : "<< bspline_interval_ << endl;
        // traj.getTimeSpan(tm, tmp);
        // double t_step = (tmp - tm) / ((traj.evaluateDeBoorT(tmp) - traj.evaluateDeBoorT(tm)).norm() / hybrid_astar_flow_->getMapResolution());
        ...
        //   flag_occ = hybrid_astar_flow_->get2DOccupancy(traj.evaluateDeBoorT(t));
        //   if (flag_occ)
        //   {
